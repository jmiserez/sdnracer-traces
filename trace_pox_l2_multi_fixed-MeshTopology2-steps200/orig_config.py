...
start_cmd = ('''./pox.py --verbose '''
              '''openflow.spanning_tree_new openflow.discovery_new forwarding.l2_multi_new '''
...
#controllers = [ControllerConfig(start_cmd, cwd="pox/", port=6633)]
...
results_dir = "traces/trace_pox_l2_multi_fixed-%s%d-steps%s" % (topology_class.__name__, num, steps)
...
#                       invariant_check_name="check_everything",
                      ...
                      apps=apps)
raise RuntimeError("Please add this parameter to Fuzzer: fuzzer_params='traces/trace_pox_l2_multi_fixed-MeshTopology2-steps200/fuzzer_params.py'")